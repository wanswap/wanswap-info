...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://wanswap.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://wanswap.finance/#/swap?inputCurrency=${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export async function getShareValueOverTime(pairAddress, timestamps) {
  ...
  for (var brow in result?.data) {
    ...
    if (timestamp && values[index] && result.data[brow]) {
...
export const urls = {
  showTransaction: (tx) => `https://www.wanscan.org/tx/${tx}/`,
  showAddress: (address) => `https://www.wanscan.org/address/${address}/`,
  showToken: (address) => `https://www.wanscan.org/token/${address}/`,
  showBlock: (block) => `https://www.wanscan.org/block/${block}/`,
}
